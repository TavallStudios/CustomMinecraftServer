...
final class BedrockDatagramTransport {
    static final int RELIABILITY_UNRELIABLE = 0;
    static final int RELIABILITY_RELIABLE_ORDERED = 3;

    void sendAck(ChannelHandlerContext context, InetSocketAddress recipient, int sequenceNumber) {
    ...
    void sendFrame(ChannelHandlerContext context, BedrockPeerSession peerSession, ByteBuf payload, int reliability) {
    ...
    ByteBuf wrapBatch(ByteBuf packet, boolean includeCompressorHeader) {
    ...
    BedrockDatagramFrame readFrame(ByteBuf in) {
    ...
    ByteBuf reassemble(BedrockPeerSession peerSession, BedrockDatagramFrame frame) {
        if (!frame.split()) {

...
import dev.tjxjnoobie.customminecraftserver.protocol.java.auth.JavaPlayerUuidResolver;
import dev.tjxjnoobie.customminecraftserver.protocol.java.packet.JavaClientSettingsPacket;
...
public final class JavaPostLoginCoordinator {
    private final StructuredConnectionLogger logger;

    public JavaPostLoginCoordinator(StructuredConnectionLogger logger) {
        this.logger = logger;
    }

    public void beginPostLoginFlow(ChannelHandlerContext context, ConnectionSession session, String transitionReason) {
    ...
    public void handleLoginAcknowledgement(ChannelHandlerContext context, ConnectionSession session, int packetId) {
    ...
    public void handleConfiguration(ChannelHandlerContext context, ConnectionSession session, int packetId) {
    ...
    public void handlePlay(ChannelHandlerContext context, ConnectionSession session, int packetId, ByteBuf frame) {
    ...
    public void handleWaitingSettings(ChannelHandlerContext context, ConnectionSession session, int packetId, ByteBuf frame) {
        if (packetId != JavaPacketIds.PLAY_SERVERBOUND_SETTINGS) {
            failAndClose(context, session, "Unsupported Java play packet id " + packetId, Map.of("packetId", packetId));
            return;
        }
    ...
    public void handleWaitingPong(ChannelHandlerContext context, ConnectionSession session, int packetId, ByteBuf frame) {
    ...
    public void cancelTimeouts(ChannelHandlerContext context) {
        JavaChannelState.cancelSettingsTimeout(context);
        JavaChannelState.cancelPongTimeout(context);
    }
